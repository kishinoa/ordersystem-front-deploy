    ...
    export default{
        ...
        methods: {
            addCart() {
                ...
                data.forEach(p=>this.$store.dispatch("addCart", p));
            },
            async searchProduct(){
                this.productList = [];
                this.currentPage = 0;
            ...
            async scrollPaging() {
                // 현재화면높이 + 스크롤로 이동한 거리 > 전체화면높이-n(내가원하는길이)가 성립하면 추가데이터 로드
            ...
            async loadData() {
                this.isLoading = true;
                // 문자열로 직접 ?size=xx&page=yy 형태로도 조립가능하지만, params라는 객체를사용하면 파라미터형식으로 url조립
                let params={
                    size:this.pageSize,
            ...
            async createOrder() {
                try{
                    ...
                    console.log(data)
                    if(data.length>0){
                        await axios.post(`${process.env.VUE_APP_API_BASE_URL}/order/create`, data);
